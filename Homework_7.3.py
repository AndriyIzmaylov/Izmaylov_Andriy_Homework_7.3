...
#
# Розберемо перший приклад, де необхідно знайти друге входження "s" в слові "sims". Якби нам треба
# було знайти її перше входження, то тут все просто: за допомогою функції index або find ми можемо дізнатися,
...
def second_index(text, some_str):
    first_incoming_index = text.find(some_str)
    if first_incoming_index == -1:
        return None

    second_incoming_index = text.find(some_str, first_incoming_index + 1)
    return second_incoming_index if second_incoming_index != -1 else None
